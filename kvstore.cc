...
KVStore::KVStore(const std::string &dir): KVStoreAPI(dir)
{
    ...
    for(int i=0; i<num; ++i)
    {
        ...
        for(int j=0; j<numSST; ++j)
        {
            ...
            TableCache sstable(FileName);
            SSTable[i].insert(move(sstable));
        }
    }
    if(SSTable.empty())
        SSTable.emplace_back();
...
void KVStore::put(uint64_t key, const string &s)
{
    ...
    if(memTable->memory > MEMTABLE)
    {
        ...
        thread compactThread(&KVStore::compaction, this);
...
void KVStore::compaction(){
    ...
    Level[0]++;
    immutableTable->store(Level[0], path);
    string newFile = path + "/SSTable" + to_string(Level[0]) + ".sst";
    TableCache newTable(newFile);
    SSTable[0].insert(move(newTable));
    compactionForLevel(1);
...
void KVStore::compactionForLevel(int level)
{
    ...
    vector<TableCache> FileToRemoveLevelminus1;                    //记录需要被删除的文件
    vector<TableCache> FileToRemoveLevel;


    //需要被合并的SSTable
    priority_queue<TableCache, vector<TableCache>, greater<>> sortTable;

    uint64_t timestamp = 0;
    int64_t tempMin = INT64_MAX, tempMax = INT64_MIN;

    //需要合并的文件数
    int compactNum = (level-1==0)? SSTnum : SSTnum - UpperNum(level-1);
...
void KVStore::writeToFile(int level, uint64_t timeStamp, uint64_t numPair, map<int64_t, string> &newTable)
{
    ...
    SSTable[level].insert(move(newSSTable));
